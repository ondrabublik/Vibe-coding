(function (NS) {
  ...
  // --- Turbo colormap (Google Brain, 256 entries) — compact version ---
  // Approximated with piecewise linear segments for performance
  function turbo(t) {
    t = Math.max(0, Math.min(1, t));
    // Piecewise RGB segments
    const r = Math.round(255 * Math.max(0, Math.min(1,
      t < 0.25 ? 0.18 + 2.64 * t :
      t < 0.50 ? 0.84 + 0.48 * (t - 0.25) :
      t < 0.75 ? 0.96 - 1.44 * (t - 0.5) :
                 0.60 - 2.16 * (t - 0.75)
    )));
    const g = Math.round(255 * Math.max(0, Math.min(1,
      t < 0.125 ? 0.06 + 5.84 * t :
      t < 0.375 ? 0.79 + 0.32 * (t - 0.125) :
      t < 0.625 ? 0.87 - 1.44 * (t - 0.375) :
      t < 0.875 ? 0.51 - 1.60 * (t - 0.625) :
                  0.11 - 0.84 * (t - 0.875)
    )));
    const b = Math.round(255 * Math.max(0, Math.min(1,
      t < 0.25  ? 0.24 + 2.76 * t :
      t < 0.375 ? 0.93 - 2.08 * (t - 0.25) :
      t < 0.50  ? 0.67 - 2.48 * (t - 0.375) :
      t < 0.75  ? 0.36 - 0.96 * (t - 0.5) :
                  0.12 - 0.36 * (t - 0.75)
    )));
    return [r, g, b];
  }

  // --- Viridis colormap (4-point linear interpolation) ---
  const viridisCtrl = [
  ...
  function viridis(t) {
    ...
    return [
      Math.round(255 * (a[0] + f * (b[0] - a[0]))),
      Math.round(255 * (a[1] + f * (b[1] - a[1]))),
      Math.round(255 * (a[2] + f * (b[2] - a[2]))),
    ];
  }

  // --- Coolwarm (blue-white-red) ---
  const cwCtrl = [
    [0.017, 0.392, 0.722], [0.550, 0.712, 0.892],
    [0.950, 0.950, 0.950],
  ...
  const jetCtrl = [
    [0.0,   0.0,   0.5  ],   // t = 0.000  dark blue
    [0.0,   0.0,   1.0  ],   // t = 0.125
    [0.0,   1.0,   1.0  ],   // t = 0.375  cyan
    [1.0,   1.0,   0.0  ],   // t = 0.625  yellow
    [1.0,   0.0,   0.0  ],   // t = 0.875  red
    [0.5,   0.0,   0.0  ],   // t = 1.000  dark red
  ];
  const jetPos = [0.0, 0.125, 0.375, 0.625, 0.875, 1.0];
  function jet(t) {
    t = Math.max(0, Math.min(1, t));
    ...
    const last = jetCtrl[jetCtrl.length - 1];
    return [Math.round(255 * last[0]), Math.round(255 * last[1]), Math.round(255 * last[2])];
  }
  ...
  // Map a scalar value in [vmin, vmax] to RGB using given colormap name
  Colormap.map = function (v, vmin, vmax, name) {
    const t = vmax > vmin ? (v - vmin) / (vmax - vmin) : 0.5;
    return (fns[name] || fns.turbo)(t);
  };

  // Pre-build a lookup table for speed (256 entries)
  ...
  Colormap.lutLookup = function (v, lutObj) {
    const { lut, vmin, vmax } = lutObj;
    const t = vmax > vmin ? (v - vmin) / (vmax - vmin) : 0.5;
